...
def handle_data(data, meta_data, info_data, flip_output=False):
    ...
    res = __handle_data(input_data=input_data, length_in_decimal=data.length_in_decimal,
...
def __handle_data(input_data, length_in_decimal, value_in_ascii, one_liner, non_tlv_neighbor, info_data):
    """
    ...
    tlv_handler_result = TlvHandler(input_data=input_data, non_tlv_neighbor=non_tlv_neighbor,
                                    info_data=info_data).process_tlv()
